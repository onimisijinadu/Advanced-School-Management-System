...
export const ClassDetails = () => {
  ...
  const selectedClass = ScheduleData.find((data) => data.id.toString() === id);
  return (
    <>
      <div className="min-h-screen min-w-7/12">
        <div className="overflow-hidden">
          <div
            className="flex gap-3 font-semibold text-slate-900 hover:bg-gray-300  rounded-lg w-max p-2.5 cursor-pointer"
            onClick={() => navigate(-1)}
          >
          ...
          {selectedClass && (
            <ClassCard
              ...
              articlestyle={
                "flex flex-wrap sm:flex-nowrap lg:flex-nowrap justify-between w-full bg-white border border-gray-300 p-5 rounded-lg gap-2"
              }
            ...
            tableHeads={
              <>
                <tr className=" border-b border-slate-200">
                  ...
                  <th className="p-3 text-slate-700 text-center">Attendance</th>
                  <th className="p-3 text-slate-700">Grade</th>
                  <th className="p-3 text-slate-700">Action</th>
                </tr>
              </>
            }
            tableBody={studentList.map((data) => (
              <tr className="border-b border-slate-200" key={data.studentId}>
          ...
          <footer className="my-5 gap-3 card-grid">
            <div className="flex flex-col items-center bg-white border border-slate-200 rounded-lg p-5 gap-2 w-full ">
              <p className="text-slate-700">Average Attendance</p>
            ...
            <div className="flex flex-col items-center bg-white border border-slate-200 rounded-lg p-5 gap-2 w-full ">
              <p className="text-slate-700">Average Grade</p>
              <h3 className="font-semibold text-2xl">A-</h3>
              <p className="text-green-600 flex items-center gap-1">
                {/* {`${improved}? ${(
                  <LucideArrowUp className={`w-2.5 h-4 text-green-600`} />
                )}:${(
                  <LucideArrowDown className={`w-2.5 h-4 text-green-600`} />
                )}`} */}
                <LucideArrowUp className={`w-2.5 h-4 text-green-600`} />
                Imporved
              </p>

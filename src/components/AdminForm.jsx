import { GraduationCap, MailIcon, LockIcon, ArrowRight } from "lucide-react";
  ...
  return (
    <>
      <div className="flex flex-col h-max w-3/4 lg:w-115 border-none bg-amber-50 border-gray-300 rounded-lg shadow-lg mx-auto mt-10 p-5 py-5">
        <div>
          <GraduationCap className="w-16 h-16 text-white mx-auto bg-blue-400 p-2 rounded-2xl my-5" />
          <h1 className="text-2xl font-medium text-center">Manage Portal</h1>
          <p className="text-gray-500 text-center">Administrative Access</p>
        </div>
        <div>
        ...
        <button
          onClick={btnclick}
          className="flex justify-center items-center font-semibold gap-1 mt-3 bg-transparent text-primaryBlue rounded-lg p-2 cursor-pointer hover:text-primaryBlue2"
        >
          <p>Switch to Teacher Login</p>
          <ArrowRight className="h-4 w-3 pt-1" />
        </button>
      </div>
    </>
  );
